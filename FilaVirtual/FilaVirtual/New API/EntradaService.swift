...
class EntradaService: ObservableObject {
    ...
    func postentrada(_ obj: Entrada){ //Aqui deve ser colocado o nome da Struct relacionada
        ...
        guard let url = URL(string: "http://192.168.128.31:1880/postentrada") else { return } //Aqui deve ser colocado o IP (local ou da rede) e, depois da barra, o verbo do POST do Node-RED
    ...
    func getentrada(){
        ...
        guard let url = URL(string: "http://192.168.128.31:1880/getentrada")else{ //Aqui deve ser colocado o IP (local ou da rede) e, depois da barra, o verbo escolhido no fluxo do GET no Node-RED
            return
        }
